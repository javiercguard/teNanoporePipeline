...
insBed = f"{outputDir}/{fileName}.rm.bed"
...
with open(inputVcf) if inputVcf.endswith(".vcf") else gzip.open(inputVcf, "rt") as fileIterator, \
    ...
    for i in range(len(lines)):
        ...
        insertions[insId] = {
            "chrom": chrom,
            "pos": str(int(pos) - 1),
            # "alt": alt, # commented stuff will be added later
            # "info": info,
...
repeatMaskerCommand = ["RepeatMasker", "-e", "rmblast", "-species", "human", "-no_is",
    "-pa", str(max(1, 10)), 
    outFasta]
