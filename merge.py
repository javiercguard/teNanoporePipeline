...
import os, sys, gzip, subprocess, argparse, traceback
...
parser.add_argument("--consider-sequence", dest = "considerSeq", default = False, action='store_true', help = "Whether to take the ALT field into consideration")
...
for i in range(len(vcfs) - 1):
    ...
    for var in vcf.fetch():
        ...
        for j in range(len(vcfs)):
            ...
            for candidate in other.fetch(var.chrom, max(var.pos - distance, 0), var.pos + distance):
                uid2 = sampleToCompare + "_" + candidate.id
                if uid2 in consumed:
                    continue
                candidates.append(candidate)
            
            # a single VCF will result in a single candidate
            candidates = list(
                filter(
                    lambda x: 
            ...
            candidates = sorted(candidates, key = lambda x: 
                        abs(getInfoValue(x, toInt = 1) - var.info["SVLEN"])) # !
            if considerSeq:
                alns = [
                    {
                        "candidate": x, 
                ...
                # # print(alns)
                chosen = max(alns, key = lambda x: x["aln"].score) # for ties would be interesting to check length diff
                ...
                # The filtering out by size difference has already been done
                # so no point repeating it
            
            uid2 = sampleToCompare + "_" + cand.id
            consumed.add(uid2)
            result["info"]["SUPP"] += 1
            result["info"]["SUPP_VEC"] = replaceChar(result["info"]["SUPP_VEC"], "1", j)
            readSupport = 0 
            ...
            result["samples"][sampleToCompare] = {
                "GT": "./.",
                "PSV": cand.info['SUPP_VEC'] if "SUPP_VEC" in cand.info.keys() else "NAN",
                ...
                "SC": chosen["aln"].score if isinstance(chosen["aln"], tuple) else chosen["aln"],
                'start': cand.pos,
                'end': cand.stop,
            }
        
        startToUse = median([x["start"] for x in result["samples"].values() if "start" in x.keys()])
        endToUse = median([x["end"] for x in result["samples"].values() if "end" in x.keys()])
        lenToUse = floor(
            median(
                [x["LN"] for x in result["samples"].values() if x["LN"]]
            )
        )
        starts = [x["start"] for x in result["samples"].values() if "start" in x.keys()]
        maxDiffInCoords = max(starts) - min(starts)
        result["info"]["MAXDIFF"] = maxDiffInCoords
        result["info"]["SVLEN"] = lenToUse
        record = out.new_record(contig = var.chrom, start = startToUse - 1, 
            stop = endToUse,
...
subprocess.run(["bcftools", "sort", outFile, "-O", "z", "-o", f"{outFile}.gz"])
subprocess.run(["bcftools", "index", "-f", f"{outFile}.gz"])
